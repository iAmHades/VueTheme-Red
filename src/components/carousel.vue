  ...
  export default {
    ...
    components: {
      indicator: {
        ...
        template: '<li v-for="i in indicator" @click="handleIndicatorClick($index)"'+
                  ' v-bind:class="{\'active\':$index === activeIndex}"><span></span></li>',
        methods: {
    ...
    methods: {
      slide(direction, selected, prev) {
        ...
        this._prevSelectedEvent = this.$listen(prevSelectedEl, 'transitionend', transitionendFn);
        this._selectedEvent = this.$listen(selectedEl, 'transitionend', transitionendFn);
