  ...
  <div class="cov-vue-date">
    <div class="datepickbox">
      <input
        type="text"
        title="input date"
        class="cov-datepicker form-control"
      ...
      <div
        ...
        <div class="cov-date-box list-box" v-if="showInfo.year">
          <div class="cov-picker-box date-list" id="yearList">
            <div class="date-item" v-for="yearItem in library.year" track-by="$index" @click="setYear(yearItem)">
              {{yearItem}}
            </div>
          </div>
        </div>
        <div class="cov-date-box list-box" v-if="showInfo.month">
          <div class="cov-picker-box date-list">
            <div class="date-item" v-for="monthItem in library.month" track-by="$index" @click="setMonth(monthItem)">
              {{monthItem}}
            </div>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      nextMonth(type) {
        ...
        type === 'next' ? next = moment(this.checked.currentMoment).add(1, 'M') : next
          = moment(this.checked.currentMoment).add(-1, 'M');
...
//        console.log(firstDay)
        ...
        const currentMonth = _.cloneDeep(currentMoment);
        const previousMonth = _.cloneDeep(currentMoment);
        const nextMonth = _.cloneDeep(currentMoment);
        ...
        for (let i = 1; i <= monthDays; ++i) {
          ...
          if (i === Math.ceil(moment(currentMoment).format('D')) && moment(oldtime).year()
            === moment(currentMoment).year() && moment(oldtime).month()
      ...
      limitWeekDay(limit, days) {
        ...
        if (limit.available.indexOf(Math.floor(moment(this.checked.year + '-' +
            this.checked.month + '-' + tday).format('d'))) === -1) {
      ...
      limitFromTo(limit, days) {
        days.map((day) => {
          if (!moment(this.checked.year + '-' + this.checked.month + '-' +
          day.value).isBetween(limit.from, limit.to)){
          day.unavailable = true;
          }
        return this;
        });
      ...
      addYear() {
        ...
        listDom.addEventListener('scroll', () => function () {
          ...
          if (listDom.scrollTop < listDom.scrollHeight - 100) {
            const len = self.library.year.length;
            const lastYear = self.library.year[len - 1];
      ...
      picked(){
        const ctime = this.checked.year + '-' + this.checked.month + '-' +
          this.checked.day + ' ' + this.checked.hour + ':' + this.checked.min;
